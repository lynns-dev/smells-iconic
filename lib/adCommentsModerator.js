...
export async function runModerationPass() {
  const postIds = monitoredPostIds();
  if (!postIds.length) {
    return { ranAt: new Date().toISOString(), postsScanned: 0, commentsSeen: 0, actions: [], error: 'No post IDs configured in META_MONITORED_POST_IDS.' };
  }
